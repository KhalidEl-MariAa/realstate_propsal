...
import { Check, ClipboardList, CreditCard, FileText, LayoutDashboard, RefreshCcw, ShieldCheck, UserRoundCheck } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800" dir="rtl">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 md:py-14">
        ...
        <section className="mb-10 rounded-[2rem] border border-amber-200 bg-amber-50 p-7 shadow-sm md:p-10"><div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between"><div className="max-w-3xl"><Eyebrow text="التحقق من هوية المستخدمين" /><h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">خدمة خارجية عبر API</h2><p className="mt-4 leading-8 text-slate-600">تُنفذ ميزة التحقق من الهوية عن طريق مزود خدمة خارجي موثوق عبر API، ولا تدخل رسومها ضمن قيمة تطوير التطبيق. لا يشمل هذا العرض بناء خدمة تحقق مخصصة من الصفر؛ لأنها تحتاج وقتًا وميزانية مستقلين، وتُعد مشروعًا متخصصًا قائمًا بذاته للشركة.</p></div><div className="w-fit rounded-2xl bg-white p-4 text-amber-700 shadow-sm"><UserRoundCheck className="h-10 w-10" /></div></div><div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><InfoCard title="الاشتراك الشهري" value="$250 شهريًا" /><InfoCard title="مدة التعاقد" value="12 شهرًا كحد أدنى" /><InfoCard title="المشمول شهريًا" value="500 عملية تحقق" /><InfoCard title="بعد الحد المشمول" value="$1.50 لكل عملية إضافية" /></div><p className="mt-5 text-sm leading-6 text-slate-500">رسوم مزود خدمة التحقق تُدفع مباشرةً للخدمة الخارجية، ويُضاف فقط ربط الـ API داخل التطبيق عند اعتماد الاشتراك.</p></section>
...
function InfoCard({ title, value }: { title: string; value: string }) { return <article className="rounded-2xl border border-amber-200 bg-white p-5"><p className="text-sm font-medium text-slate-500">{title}</p><p className="mt-3 text-lg font-semibold text-slate-950">{value}</p></article>; }
